...
| 2 | Panels live outside the tabset (in the panel layer), so the tabset's `border-radius`/`overflow` cannot clip them, and a panel cannot be styled by its tabset's state. | The panel repeats the tabset's inner radius on its bottom corners (`rounded-b-[calc(var(--radius-md)-1px)]`). It works, but it duplicates a value the tabset owns, and it breaks if the tab strip moves to the bottom. | `data-tabset` (the tabset path) and `data-tabset-active` on `Dockable.Panel`, plus the same keys in `PanelState`. |
...
| 9 | A wider splitter **hit area** than its visible thickness (a 1px line that is easy to grab). | Not done. It is possible in consumer CSS with a positioned pseudo-element, since the engine measures the element, not the pseudo-element. | Document the pseudo-element pattern; optionally a `Dockable.SplitterHandle` part. |
