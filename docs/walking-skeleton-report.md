...
| 2 | Panels live outside the tabset (in the panel layer), so the tabset's `border-radius`/`overflow` cannot clip them, and a panel cannot be styled by its tabset's state. | None. The square panel corners show inside the rounded tabset. | `data-tabset` (the tabset path) and `data-tabset-active` on `Dockable.Panel`, plus the same keys in `PanelState`. |
