...
## Not rendered yet: borders, floats and tab groups

The model applies these (they are ported from FlexLayout and unit-tested), but no primitive renders borders, floats or tab groups yet, so their effect is not visible. See [Limitations](/docs/limitations).
