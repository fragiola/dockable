...
- The element: `display: flex` (a row for left and right borders, a column otherwise) or
  ...
  right overlay stops above and below the open top and bottom overlays. An overlay is also
  `pointer-events: none`, so presses reach the tab panel positioned over its area; its splitter
  (`Dockable.Splitter` of a border is `pointer-events: auto`) keeps taking them. Hit-testing, not
  cosmetics.
