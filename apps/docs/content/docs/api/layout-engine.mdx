...
| `dockBack(node)` | moves a tab (or every tab of a tabset) from a window into the main layout's active tabset (else its first, else a new one); emptying a window closes it |
...
| `getDragGroup()` | the [drag group](/docs/guides/cross-layout-drag) this layout exchanges tabs in (the main engine's), if any |
