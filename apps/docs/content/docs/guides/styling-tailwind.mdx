...
- **Give tab strips a minimum height** (`min-h-7` or so), or lower the global `edgeDockMargin`:
  a very thin strip at the top of the layout falls inside the 10px top-edge docking band (gap 11).
