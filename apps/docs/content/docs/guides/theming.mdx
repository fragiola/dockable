...
of the page, but **not** other attributes of `<html>` and `<body>`. If your theme keys off
`:root[data-theme]` or a body class, copy them in `onOpen`:
...
To follow later theme switches, keep the popout documents (`onOpen`/`onClose`) and re-apply
with a `MutationObserver`, as the examples' kit does. This is gap 3 in
[Limitations](/docs/limitations); Epic [#20](https://github.com/fragiola/dockable/issues/20)
adds a `mirrorRoot` option.
