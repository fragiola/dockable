// Ported from FlexLayout (https://github.com/caplin/FlexLayout), tests/OverlayBorder.test.ts.
// Copyright (c) 2017 Caplin Systems Ltd. MIT licence, see LICENSE.
import { beforeEach, describe, expect, it } from "vitest";
import { Actions, type IJsonModel, Model } from "../../src";
...
describe("setBorderType action", () => {
    ...
    it("is independent of enableAutoHide", () => {
        ...
        model.doAction(
            Actions.updateNodeAttributes("border_left", {
                enableAutoHide: true,
            } as any),
        );
...
describe("serialization", () => {
    it("round-trips the borderType attribute", () => {
        ...
        expect(
            json2.borders!.find((b) => b.location === "bottom")!.borderType,
        ).equal(undefined); // default not written

        const model2 = Model.fromJson(json);
        expect(
            (model2.getNodeById("border_left") as BorderNode).isOverlay(),
        ).equal(true);
    });
});
