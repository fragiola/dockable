// Ported from FlexLayout (https://github.com/caplin/FlexLayout), tests/KeyMap.test.ts.
...
describe("matchesKey", () => {
    ...
    it("requires exactly the specified modifiers", () => {
        expect(
            matchesKey(keyEvent("Delete", { ctrlKey: true }), "Ctrl+Delete"),
        ).toBe(true);
        expect(matchesKey(keyEvent("Delete"), "Ctrl+Delete")).toBe(false);
        expect(
            matchesKey(
                keyEvent("Delete", { ctrlKey: true, shiftKey: true }),
                "Ctrl+Delete",
            ),
        ).toBe(false);
        expect(
            matchesKey(keyEvent("Delete", { ctrlKey: true }), "Delete"),
        ).toBe(false);
    });
    ...
    it("supports multiple modifiers in any order", () => {
        ...
        expect(
            matchesKey(
                keyEvent("w", { altKey: true, metaKey: true }),
                "Alt+Meta+W",
            ),
        ).toBe(true);
    });
    ...
    it("never matches an undefined or empty spec (a disabled binding)", () => {
        expect(
            matchesKey(keyEvent("Delete", { ctrlKey: true }), undefined),
        ).toBe(false);
...
describe("resolveKeyMap", () => {
    ...
    it("merges configured bindings over the defaults", () => {
        ...
        expect(resolved.closeOverlayBorder).toBe(
            defaultKeyMap.closeOverlayBorder,
        );
    });
    ...
    it("focusTabToggle is off by default and enabled by configuring a binding", () => {
        ...
        expect(resolveKeyMap({ focusTabToggle: "F6" }).focusTabToggle).toBe(
            "F6",
        );
    });
    ...
    it("tabset cycling is off by default and enabled by configuring bindings", () => {
        ...
        const resolved = resolveKeyMap({
            focusNextTabset: "Ctrl+ArrowRight",
            focusPreviousTabset: "Ctrl+ArrowLeft",
        });
...
describe("hasModifier", () => {
    it("is true when any modifier is held", () => {
        expect(hasModifier(keyEvent("ArrowLeft"))).toBe(false);
        expect(hasModifier(keyEvent("ArrowLeft", { ctrlKey: true }))).toBe(
            true,
        );
        expect(hasModifier(keyEvent("ArrowLeft", { shiftKey: true }))).toBe(
            true,
        );
        expect(hasModifier(keyEvent("ArrowLeft", { altKey: true }))).toBe(true);
        expect(hasModifier(keyEvent("ArrowLeft", { metaKey: true }))).toBe(
            true,
        );
    });
});
